...
class Project(Base):
    ...
    #
    # ML-1 (fixed): this field is now populated for Phase-2-imported rows
    # with one of COMPLETED / ONGOING / SANCTIONED / RECOMMENDED /
    # NOT_SPECIFIED, sourced by import_phase2.py from the canonical Phase
    # 2 lifecycle status computed by ml.status.resolve_status_frame()
    # (see ml/status.py, ml/canonical.py). That resolution reconciles the
    # Sanctioned-stage `work_status` text (e.g. "Physical Inspection",
    # "Vendor Identification") against the Works Completed dataset, so a
    # project whose work_status looks in-progress but already has a
    # matching completion record correctly resolves to COMPLETED here.
    # The raw work_status value itself is a different, non-equivalent
    # vocabulary and is preserved as-is in risk_metadata for reference,
    # never overwritten. Previously (pre-ML-1) this column was left NULL
    # because no verified crosswalk existed; it may still be NULL for a
    # row whose work_id isn't present in the current canonical dataset.
