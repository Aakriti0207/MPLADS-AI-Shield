...
const PAGE_SIZE = 50
// Pages kept in memory for this visit to the page (see cacheRef below).
...
export default function Projects() {
  ...
  // `loading` = nothing to show yet (first load only). `refreshing` = a newer
  // page is being fetched while the previous one stays on screen.
  const [loading, setLoading] = useState(true)
  const [refreshing, setRefreshing] = useState(false)
  const [reloadToken, setReloadToken] = useState(0)
  // Per-visit page cache. It lives in a ref, so it disappears with this
  // component: nothing scope-specific survives a logout or user switch.
  const cacheRef = useRef(new Map())
  const apiMode = useDemoApi ? 'demo' : useProtectedApi ? 'protected' : 'public'
  const openProject = useCallback((url) => navigate(url), [navigate])
  ...
  //    instead of the table being replaced by a spinner on every click;
  //  - superseded requests are aborted;
  //  - the following page is prefetched, so "Next" is instant;
  //  - pages already seen during this visit are served from memory.
  useEffect(() => {
    if (!authReady) return
    ...
    const fetcher = useDemoApi
      ...
      : useProtectedApi
        ? fetchProjectPage
        : fetchPublicProjectPage
    const keyFor = (f) => `${apiMode}|${JSON.stringify(f)}`
    const totalOf = (data) =>
      data.total ?? data.total_count ?? data.count ?? null
    ...
    const prefetchNext = (data) => {
      const count = (data.items || []).length
      const known = totalOf(data)
      if (count < PAGE_SIZE) return
      if (known !== null && skip + PAGE_SIZE >= known) return
      const nextFilters = { ...filters, skip: skip + PAGE_SIZE }
      const nextKey = keyFor(nextFilters)
      if (cache.has(nextKey)) return
      prefetchTimer = setTimeout(() => {
        fetcher(nextFilters)
          .then((next) => remember(nextKey, next))
          .catch(() => {
    ...
    setRefreshing(true)
    fetcher(filters, { signal: controller.signal })
      .then((data) => {
        if (cancelled) return
        remember(key, data)
        applyPage(data)
