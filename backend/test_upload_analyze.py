...
def admin_auth_headers(client, db_session):
    """Log in as a genuine Administrator user and return Authorization headers.

    Phase 7: POST /auth/register always downgrades a requested
    "Administrator" role to DEFAULT_REGISTRATION_ROLE (see
    resolve_registration_role in app/auth.py and tests/test_rbac.py) --
    that's a deliberate anti-privilege-escalation rule, not a bug, so
    this fixture can't go through the public registration endpoint the
    way `registered_user`/`auth_headers` do. Instead it inserts the
    User row directly (the same way an operator would provision a real
    Administrator account -- see resolve_registration_role's own
    docstring), then logs in normally through POST /auth/login so the
    returned token is a real, backend-issued JWT.
    """
    from app.auth import hash_password
    from app.models import User

    email = "admin.test@example.gov.in"
    password = "SecurePass123"
    db_session.add(User(email=email, password_hash=hash_password(password), role="Administrator"))
    db_session.commit()
