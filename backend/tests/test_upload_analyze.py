...
Uses the same `client` / `admin_auth_headers` / `db_session` fixtures as the
...
def test_upload_with_valid_jwt_succeeds(client, admin_auth_headers):
    resp = _upload(client, admin_auth_headers, VALID_CSV)
    assert resp.status_code == 200


# --- File-level validation --------------------------------------------------

def test_upload_empty_file_is_rejected(client, admin_auth_headers):
    resp = _upload(client, admin_auth_headers, "")
    assert resp.status_code == 400


def test_upload_wrong_file_extension_is_rejected(client, admin_auth_headers):
    resp = _upload(client, admin_auth_headers, VALID_CSV, filename="projects.txt")
    assert resp.status_code == 400


def test_upload_missing_required_column_is_rejected(client, admin_auth_headers):
    csv_without_work_id = "state,sanctioned_amount\nBihar,500000\n"
    resp = _upload(client, admin_auth_headers, csv_without_work_id)
    assert resp.status_code == 400
    assert "work_id" in resp.json()["detail"]


def test_upload_malformed_csv_is_handled_cleanly(client, admin_auth_headers):
    ...
    resp = _upload(client, admin_auth_headers, malformed)
    assert resp.status_code in (400, 200)  # pandas' C parser can recover some ragged rows
    assert resp.status_code != 500


def test_upload_duplicate_work_id_in_file_is_rejected(client, admin_auth_headers):
    dup_csv = (
        "work_id,sanctioned_amount\n"
        "WS/DUP/1,100000\n"
        "WS/DUP/1,200000\n"
    )
    resp = _upload(client, admin_auth_headers, dup_csv)
    assert resp.status_code == 400


def test_upload_too_many_rows_is_rejected(client, admin_auth_headers):
    header = "work_id,sanctioned_amount\n"
    rows = "".join(f"WS/BULK/{i},100000\n" for i in range(MAX_ROWS + 1))
    resp = _upload(client, admin_auth_headers, header + rows)
    assert resp.status_code == 400


# --- Row-level validation: bad rows never crash the whole upload ---------

def test_upload_invalid_numeric_value_is_a_row_level_error(client, admin_auth_headers):
    csv_content = (
        "work_id,sanctioned_amount\n"
        "WS/GOOD/1,500000\n"
        "WS/BAD/1,not-a-number\n"
    )
    resp = _upload(client, admin_auth_headers, csv_content)
    assert resp.status_code == 200
    body = resp.json()
...
def test_upload_invalid_date_value_is_a_row_level_error(client, admin_auth_headers):
...
def test_upload_blank_work_id_is_a_row_level_error(client, admin_auth_headers):
...
def test_upload_runs_real_compliance_rules_and_flags_expected_issues(client, admin_auth_headers):
...
def test_upload_never_returns_a_fabricated_risk_score(client, admin_auth_headers):
    resp = _upload(client, admin_auth_headers, VALID_CSV)
    assert resp.status_code == 200
    row = resp.json()["results"][0]
...
def test_upload_missing_dates_produce_not_evaluable_not_a_crash(client, admin_auth_headers):
...
def test_upload_flags_work_id_that_already_exists_in_database(client, db_session, admin_auth_headers):
...
def test_upload_new_work_id_does_not_match_existing_project(client, admin_auth_headers):
    resp = _upload(client, admin_auth_headers, VALID_CSV)
    row = resp.json()["results"][0]
...
def test_upload_does_not_persist_to_database(client, db_session, admin_auth_headers):
...
def test_upload_does_not_modify_existing_project_row(client, db_session, admin_auth_headers):
    db_session.add(Project(project_id="WS/UNCHANGED/1", sanctioned_amount=Decimal("999.00"), is_synthetic=True))
    db_session.commit()

    csv_content = "work_id,sanctioned_amount\nWS/UNCHANGED/1,1.00\n"
    _upload(client, admin_auth_headers, csv_content)
...
def test_projects_dashboard_alerts_still_work_after_upload_endpoint_added(client, db_session, admin_auth_headers):
    db_session.add(Project(project_id="WS/REGRESSION/1", risk_level="LOW", is_synthetic=True))
    db_session.commit()

    assert client.get("/projects", headers=admin_auth_headers).status_code == 200
    assert client.get("/dashboard/stats", headers=admin_auth_headers).status_code == 200
    assert client.get("/alerts", headers=admin_auth_headers).status_code == 200