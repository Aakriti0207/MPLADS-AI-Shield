...
class PublicProjectOut(BaseModel):
    ...
    Deliberately excludes risk_score/risk_level/risk_reason_*/risk_metadata
    and any review/internal/stakeholder-private data. Everything below is
    already public-facing MPLADS information (work identity, location,
    financials, lifecycle dates) -- see the Overview fix's spec: "MP if
    already public and appropriate" and "available public dates" are both
    explicitly allowed. Added district/mp_name/implementing_agency and the
    four lifecycle dates (previously missing here, which made the public
    Project Detail page show "Not available" for fields the authenticated
    view has) -- these are existing Project columns, not invented ones.
    """
